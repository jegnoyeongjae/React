...
const DetailPage = ({ Data }) => {
  ...
  */
  const fdate = findData?.date;
  const fr = new Date(fdate).getMonth() + 1;
  console.log('날짜확인1', fdate);
  console.log('날짜확인2', fr);

  const findOData = Data.filter((othData) => {
    const odate = new Date(othData.date);
    const res = odate.getMonth() + 1 === fr;
    return res;
  });

  console.log('findo', findOData);

  const handleClickPrev = () => {
    if (currentIndex < findOData.length - 1) {
